...
- **전자앨범** - microSD 카드의 `.bmp` / `.jpg` / `.png` 이미지를
  ...
  설정 가능. `.gif` 파일은 화면에 맞춰 늘리지 않고 원본 크기 그대로
  가운데에서 애니메이션으로 재생됩니다. 최대 4개의 Flickr RSS/Atom 피드
  사진도 자동으로 동기화해서 함께 재생할 수 있습니다.
...
- **완전 웹 기반 설정** - 기기가 직접 제공하는 작은 HTML5/CSS/JS 페이지:
  ...
  - 밝기, 자동 순환, 음소거.
  - 다크/라이트 테마, 또는 지정한 시각에 맞춰 자동으로 전환되는 Auto 모드
    (예: 07:00부터 라이트, 20:00부터 다크).
  - 전자앨범 재생 간격/셔플, Flickr 피드 (추가/삭제, 즉시 동기화).
  - 날씨: OpenWeatherMap API 키 + 도시 ID, 화면 켜기/끄기.
  - 호스트명, 재시작, 공장 초기화, 폴백 AP 비밀번호 설정.
  - 시리얼 콘솔 로그 레벨(None~Verbose, 기본값 Info) - 재플래시 없이 바로
    적용됩니다.
  - **파일 관리자**: 브라우저에서 바로 microSD 카드의 파일을 탐색, 업로드,
    다운로드/보기, 이름변경, 삭제할 수 있습니다 (사진을 넣기 위해 별도
    앱이 필요 없습니다). 카드를 완전히 지워야 할 때를 위한 원클릭 **포맷**
    기능도 있습니다. 업로드는 여러 파일을 한 번에 선택하거나, 파일들을
    드래그 앤 드롭으로 올릴 수도 있습니다.
...
`.jpg`/`.jpeg`, `.png`, `.gif` 파일을 넣으세요 (`/photos/*.bmp`). 정적
이미지(bmp/jpg/png)는 480x320 화면에 맞게 자동으로 스케일링되며, 최상의
화질을 원한다면 미리 480x320으로 편집해서 넣는 것을 권장합니다. BMP는 외부
의존성이 전혀 없어 항상 동작이 보장되는 포맷이며, JPEG/PNG는 각각
`espressif/esp_jpeg`, `espressif/libpng` 관리형 컴포넌트를 통해 디코딩됩니다
(아래 "알려진 한계" 참고). PNG의 투명도는 무시됩니다(불투명하게 처리) -
사진이 화면 전체를 채우기 때문에 뒤에 비칠 배경이 없기 때문입니다.

`.gif` 파일은 위 정적 이미지와는 다른 경로로, LVGL 자체 디코더를 통해
애니메이션으로 재생됩니다 - 화면에 맞춰 늘리지 않고 **원본 해상도 그대로**
가운데에 표시됩니다. 부드럽게 재생되고 프레임 버퍼 메모리를 너무 많이
쓰지 않도록 픽셀 크기와 파일 크기 모두 적당히 작게 유지하세요.

#### Flickr 피드

**Album** 탭에서 Flickr 사진 피드(RSS 또는 Atom)를 최대 4개까지 추가할 수
있습니다. 예를 들어 특정 사용자의 공개 사진은
`https://www.flickr.com/services/feeds/photos_public.gne?id=<사용자ID>&format=rss2`,
태그는 `...?tags=<태그>` 형식입니다. 기기가 Wi-Fi에 연결되면 각 피드의
이미지가 SD 카드의 `/photos/flickr/<피드ID>/` 폴더로 다운로드되어
슬라이드쇼에 포함됩니다. 피드는 1시간마다 다시 확인하며, *Sync now* 버튼을
누르거나 피드를 추가/삭제하면 즉시 동기화합니다. 이 폴더는 피드와 똑같이
유지됩니다 - 피드에서 빠진 사진은 삭제되고, 피드를 삭제하면 그 피드의 사진도
모두 삭제되므로 `/photos/flickr` 아래에는 다른 파일을 두지 마세요. 이미지는
피드 기본값인 1024px 대신 Flickr의 640px 크기로 받아서, 화면을 채우기엔
충분하면서도 다운로드와 디코딩 부담을 줄입니다.
...
components/
  ...
  app_flickr/          Flickr RSS/Atom 피드 -> SD 카드 사진 동기화
...
PR을 보내기 전에 코딩 스타일과 프로젝트 컨벤션을 정리한
[CONTRIBUTING.md](CONTRIBUTING.md) (영문)를 참고하세요.
