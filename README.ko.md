...
2. 정보 화면(스와이프로 이동)에 `WT32-A1B2C3` (MAC 주소 뒤 3옥텟) 같은
   SoftAP 이름이 표시되며, 기본값으로 **비밀번호가 없습니다**(오픈 네트워크).
   휴대폰이나 노트북을 이 AP에 연결하면 대부분 자동으로 캡티브 포털 창이
   뜨고, 뜨지 않으면 `http://192.168.4.1/` 을 직접 엽니다.
...
4. SoftAP과 캡티브 포털은 항상 켜져 있으므로, 홈 네트워크가 끊기더라도
   ...
   `http://<기기IP>/` 또는 `http://wt32.local/` 로도 접속 가능합니다. 오픈
   AP가 계속 켜져 있는 게 불안하시면 **System** 탭에서 비밀번호를 설정할
   수 있습니다.
